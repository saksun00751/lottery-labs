...
import { useSubmitSlip, useWallet } from '@/lib/api/queries';
...
export function BetSlipPanel({ round }: { round: LotteryRound }) {
  ...
  const maxWin = entries.reduce(
    (sum, entry) => sum + Math.round(entry.stake * entry.payout),
    0,
  );
  ...
  return (
    <>
      <div className={styles.bar}>
        ...
        <div className={styles.barInfo}>
          <span className={styles.barLabel}>{t('maxWin')}</span>
          <span className={styles.barValue}>
            <Money value={maxWin} size="md" tone="success" compact />
          </span>
        </div>
        <Button
        ...
        {entries.length === 0 ? (
          ...
          <>
                ...
                <div key={entry.key} className={styles.entry}>
                  ...
                  <div className={styles.entryStake}>
                    <input
                      ...
                      aria-label={t('stake')}
                      onChange={(event) =>
                        updateStake(entry.key, parseAmountInput(event.target.value))
                      }
                    />
                  </div>
                  <button
            ...
            <div className={styles.totals}>
              ...
              <div className={styles.totalRow}>
                <span>{t('maxWin')}</span>
                <Money value={maxWin} size="md" tone="success" />
              </div>
            </div>
